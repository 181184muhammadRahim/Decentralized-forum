...
function Post(props) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setForum(forum);
      try {
        let a = await forum.methods.reactionExists(props.post.id).call();
        console.log(a);
        setDisable(a);
      } catch (e) {
        console.log("ereeee", e);
      }
      //setDisable(a);
    }
        ...
        {can_view ? (
          <div>
            <ViewComment
              ...
              forum={forum}
            ></ViewComment>
            <button className="btn btn-dark" onClick={() => setView(false)}>
              Back
            </button>
          </div>
        ) : (
          <div className="cardValue px-4 py-5">
            ...
            <p className="messagePost text-left ml-5 mt-3">
              {props.post.message}
            </p>
            ...
            <div className="text-left ml-5">
              <button
                className="btn btn-primary buttons mr-1"
                disabled={disable}
                onClick={likepost}
              >
                Like Post
              ...
              <button
                className="btn btn-danger buttons mx-1"
                disabled={disable}
                onClick={dislikepost}
              >
                Dislike Post
              ...
              <button
                className="btn btn-dark buttons mx-1"
                onClick={() => {
                  console.log("here");
                  setView(true);
                }}
